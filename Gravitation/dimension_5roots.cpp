...
int coloring(double i, double j, double roots[5][2], float ratio) {
    ...
    if (distances[0] < distances[1] && distances[0] < distances[2] && distances[0] < distances[3] && distances[0] < distances[4]) {
        return 1;
    }
    if (distances[1] < distances[0] && distances[1] < distances[2] && distances[1] < distances[3] && distances[1] < distances[4]) {
        return 2;
    }
    if (distances[2] < distances[0] && distances[2] < distances[1] && distances[2] < distances[3] && distances[2] < distances[4]) {
        return 3;
    }
    if (distances[3] < distances[0] && distances[3] < distances[1] && distances[3] < distances[2] && distances[3] < distances[4]) {
        return 4;
    }
    if (distances[4] < distances[0] && distances[4] < distances[1] && distances[4] < distances[2] && distances[4] < distances[3]) {
        return 5;
    }
...
int main()
{
    ...
    double roots[5][2] = { {1,0}, {cos(2 *  M_PI/ 5),sin(2 * M_PI / 5)} , {cos(4 * M_PI / 5),sin(4 * M_PI / 5)} , {cos(6 * M_PI / 5),sin(6 * M_PI / 5)} , {cos(8 * M_PI / 5),sin(8 * M_PI / 5)} };
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++)
        {
            ...
            for (int h = 0; h < iteration; h++) {
                grad = Ctime(Cadd(Ctime(Ctime(Ctime(Ctime(z, z), z), z), z), Cmult(one, -1)),
                    Creciprocal(Ctime(Ctime(Ctime(Cmult(z, 5), z), z), z)));
    ...
    image.Export("C:/Users/효서/Desktop/Fractals/Newton_dimension_5roots_2.bmp");
