    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++)
        {
            ...
            for (int h = 0; h < iteration; h++) {
                //grad = Ctime( Cadd(Cadd(Cadd(Ctime(Ctime(z, z), z), Ctime(Cmult(z, -4), z)), Cmult(z, 6)), Cmult(one, -4)),
                //    Creciprocal(Cadd(Cadd(Ctime(Cmult(z, 3), z), (Cmult(z, -8))), Cmult(one, 6))));
                
                grad = Ctime(Cadd(Cadd(Ctime(Ctime(z, z), z), Cmult(z, -2)), Cmult(one, 4)),
                        Creciprocal(Cadd(Ctime(Cmult(z, 3), z), Cmult(one, -2))));
